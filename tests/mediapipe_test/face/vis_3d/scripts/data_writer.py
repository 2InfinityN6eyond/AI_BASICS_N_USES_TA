...
'''
class DataWriter_(multiprocessing.Process) :
    def __init__(
        self,
        frame_width     : int,
        frame_height    : int,
        ...
        save_every_data_flag : bool = True,
    ...
    def run(self) :
        ...
        while not self.stop_flag.is_set() :
            if not self.save_data_queue.empty() :
                ...
                print("recieved", shm_idx, mouse_position)
                continue

                image = image_queue[shm_idx].copy()
                image = cv2.flip(image, 1)

                face_landmark_array = face_queue[shm_idx].copy()
                face_landmark_array[:, 0] = 1 - face_landmark_array[:, 0]

                timestamp = str(time.time()).replace('.', '_')
                
                print("recieved", shm_idx, mouse_position, timestamp, image.shape)
                continue
